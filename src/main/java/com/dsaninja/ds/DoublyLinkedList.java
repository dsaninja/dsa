...
/**
 * A custom linked list implementation in java. The list is a doubly linked list
...
public class DoublyLinkedList<E>{
    ...
    public void add(int index, E element){
        ...
        // one node behind new node
        Node previousToNewNode = getIthNode(index - 1);
        // one node ahead of new node
        Node nextToNewNode = previousToNewNode.next;

        Node newNode = new Node(element);
        previousToNewNode.next = newNode;
        newNode.prev = previousToNewNode;

        // inserted as non-last node
        if(null != nextToNewNode){
